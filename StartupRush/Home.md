---
tags:
  - startup-rush
  - moc
aliases:
  - MOC
  - Index
---

# Startup Rush

> Evento imersivo de 54 horas baseado nos princípios do Lean Startup.
> Previsão: 2º semestre de 2026.

---

## Metodologia

- [[Metodologia StartupRush]] — Documento central: visão, fases, frameworks, papéis
- [[Cronograma 54h]] — Agenda hora-a-hora das 54 horas
- [[Guia do Facilitador]] — Manual operacional para facilitadores e mentores
- [[Apresentação de Abertura]] — Roteiro e conteúdo do deck de abertura (20 min)
- [[Small Batches e Motores de Crescimento]] — Parte 3 do Lean Startup: lotes pequenos + 3 motores de crescimento
- [[Diagramas Visuais]] — BML, fases, early adopters, pivôs — em Mermaid + descrições para designer
- [[Programação Noturna]] — Atividades intencionais para sexta e sábado à noite
- [[Plano de Contingência]] — O que fazer quando as coisas não saem como planejado

## Templates (para equipes)

- [[Canvas de Hipóteses]] — Fase 1: mapear hipóteses de valor e crescimento
- [[Canvas de Experimento]] — Fase 2: design científico do teste
- [[Canvas SLC]] — Fase 2: definir o que construir (e o que NÃO construir) — Simples, Adorável, Completo
- [[Roteiro de Entrevista]] — Fase 2: script estruturado para "Saia do Prédio"
- [[Diário de Aprendizado]] — Fases 2-4: log de aprendizados por ciclo BML
- [[Painel de Métricas]] — Fase 3: Innovation Accounting, 3 As das métricas, análise de coorte
- [[Five Whys]] — Fase 3: técnica dos 5 Porquês para encontrar a causa raiz

## Avaliação

- [[Critérios dos Jurados]] — Rubrica de avaliação com critérios Lean (100 pts)
- [[Scorecard do Jurado]] — Ficha individual de pontuação por equipe
- [[Exemplo de Pitch Ideal]] — Pitch fictício "FeiraNow" (~88/100) como referência

## Pré-Evento

- [[Guia do Participante]] — Material enviado 7 dias antes: o que é, o que trazer, pré-tarefas
- [[Checklist Pré-Evento (Organização)]] — Tarefas da organização: 8 semanas → dia do evento
- [[Roteiro Vídeo Introdutório]] — Script de 2-3 min para vídeo de divulgação + shot list
- [[Treinamento de Facilitadores]] — Workshop obrigatório de 4h com os facilitadores
- [[Brief de Design]] — Instruções completas para designer: templates A3, slides, diagramas, divulgação

## Pós-Evento

- [[Plano de Continuidade]] — Startup Rush Labs, conexão com ecossistema, comunidade alumni

## Referências (Lean Startup)

- [[Executive Briefing - Lean Startup]] — Resumo executivo da metodologia
- [[Guia Build-Measure-Learn]] — Guia para iniciantes sobre o ciclo BML
- [[Plano de Implementação Estratégica]] — Implementação corporativa do Lean Startup
- [[Study Guide - Lean Startup]] — Study guide do livro (Intro – Cap. 7)
- [[Roteiro Lean Startup 54h]] — Roteiro original mapeando conceitos para as 54h
- [[Casos de Estudo - Lean Startup]] — IMVU, Zappos, Dropbox, Votizen, Groupon, Food on the Table adaptados
- ![[The Lean Startup - Erick Ries.pdf]] — Livro completo (PDF)

## Reuniões

- [[2026-04-10 Avaliação Marcas e Domínios]] — Análise de viabilidade do nome e domínios
- [[2026-04-10 Talk Liderança SW]] — Debate sobre desvinculação do Startup Weekend/Techstars

## Análises

- [[Avaliação da Metodologia vs Livro]] — Comparação profunda da metodologia com os princípios do livro

---

## Decisões-chave

| Dimensão | Decisão |
|---|---|
| Duração | 54 horas (sexta à noite → domingo à tarde) |
| Público | Misto — aberto a todos os perfis |
| Capacidade | 80–120 participantes |
| Imersão | Pernoite opcional (escolha do participante) |
| Metodologia | Inspirada no Lean Startup, com identidade própria |
| Formação de equipes | Pitch aberto de 60s + votação + afinidade |
| Validação externa | Incentivada fortemente, não obrigatória |
| Avaliação | Aprendizado validado > ideia brilhante |
| Previsão | 2º semestre de 2026 |

---

## Mapa de navegação

```
Home (este arquivo)
├── Metodologia/
│   ├── Metodologia StartupRush (documento central)
│   ├── Cronograma 54h
│   ├── Guia do Facilitador
│   ├── Apresentação de Abertura
│   ├── Small Batches e Motores de Crescimento
│   ├── Diagramas Visuais ⭐ NOVO
│   ├── Programação Noturna
│   └── Plano de Contingência
├── Templates/
│   ├── Canvas de Hipóteses (com Analogs/Antilogs)
│   ├── Canvas de Experimento (com Split-Testing)
│   ├── Canvas SLC
│   ├── Roteiro de Entrevista ⭐ NOVO
│   ├── Diário de Aprendizado
│   ├── Painel de Métricas ⭐ NOVO
│   └── Five Whys ⭐ NOVO
├── Avaliação/
│   ├── Critérios dos Jurados
│   ├── Scorecard do Jurado
│   └── Exemplo de Pitch Ideal ⭐ NOVO
├── Pré-Evento/
│   ├── Guia do Participante
│   ├── Checklist Pré-Evento (Organização)
│   ├── Roteiro Vídeo Introdutório ⭐ NOVO
│   ├── Treinamento de Facilitadores ⭐ NOVO
│   └── Brief de Design ⭐ NOVO
├── Pós-Evento/
│   └── Plano de Continuidade
├── Referências/
│   ├── (materiais do Lean Startup)
│   └── Casos de Estudo - Lean Startup ⭐ NOVO
└── Reuniões/
    └── (transcrições e summaries)
```
